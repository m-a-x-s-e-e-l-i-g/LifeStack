...
(movies, money, fuel, energy, mobility) is a pluggable module that owns its own sync,
storage, and dashboard. The backend aggregates; the frontend visualizes.
...
2. **Empty is a state, not a failure.** A fresh install with a module enabled but no data
   should still feel intentional, with a clear path to import or sync.
3. **Self-host ergonomics win ties.** One command up, sensible defaults, demo data on by
   default so the first run is a populated, beautiful dashboard.
